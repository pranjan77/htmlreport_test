package htmlreport_test::htmlreport_testClient;
...
htmlreport_test::htmlreport_testClient
...
sub new
{
    ...
    my $self = {
	client => htmlreport_test::htmlreport_testClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a htmlreport_test.FilterContigsParams
$output is a htmlreport_test.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is a htmlreport_test.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a htmlreport_test.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "htmlreport_test.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "htmlreport_test.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for htmlreport_test::htmlreport_testClient\n";
    }
    if ($sMajor == 0) {
        warn "htmlreport_test::htmlreport_testClient version is $svr_version. API subject to change.\n";
    }
}
...
package htmlreport_test::htmlreport_testClient::RpcClient;
